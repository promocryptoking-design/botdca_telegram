...
async def usdtbtc(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await check_group(update):
        return
...
async def dca(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        for i in range(1, niveles + 1):
            nivel_precio = precio * (1 - drop * i)
            msg += f"Nivel {i}: `{nivel_precio:.2f}`\n"
...
def main():
    ...
    app.add_handler(CommandHandler("usdtbtc", usdtbtc))
